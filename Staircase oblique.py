...
image = visual.GratingStim(win=win, name='grating',
    ...
    ori=45, pos=[-10, 1], units='cm', size=[15, 15], sf=1.14, phase=0.0,
    color=[1,1,1], colorSpace='rgb', opacity=1,
    texRes=128, interpolate=True, depth=-1.0)
image_2 = visual.GratingStim(win=win, name='grating',
    tex=u'sin', mask='circle',
    ori=0, pos=[10, 1], units='cm', size=[15, 15], sf=1.14, phase=0.0,
    color=[1,1,1], colorSpace='rgb', opacity=1,
    texRes=128, interpolate=True, depth=-1.0)
...
for thisTrial in trials:
    ...
    mouse.setPos((0,0))
    if save_correct==1 and np.abs(orientation_diff_list)>=5:
        if np.random.randint(0,100,1)>=50:
            
            orientation_diff_list=(np.abs(orientation_diff_list)-3)*-1
        else:
            orientation_diff_list=(np.abs(orientation_diff_list)-3)
            
        #print orientation_diff_list

        
    if save_correct==0 and np.abs(orientation_diff_list)<85:
        if np.random.randint(0,100,1)>=50:
            orientation_diff_list=(np.abs(orientation_diff_list)+8)*-1
        else:
            orientation_diff_list=(np.abs(orientation_diff_list)+8)
        #print orientation_diff_list
    ...
    if trialnumber<10:
        if np.random.uniform(0,high=100)<=50:
            ...
            orientation_difference=orientation_diff_list
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        save_difference=orientation_diff_list
