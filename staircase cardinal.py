 #!/usr/bin/env python2
from __future__ import division  # so that 1/3=0.333 instead of 1/3=02
...
image = visual.GratingStim(win=win, name='grating',
    ...
    ori=90, pos=[-10, 1], units='cm', size=[15, 15], sf=1.14, phase=0.0,
    color=[1,1,1], colorSpace='rgb', opacity=1,
    texRes=128, interpolate=True, depth=-1.0)
...
for thisTrial in trials:
    ...
    mouse.setPos((0,0))
    if save_correct==1 and orientation_diff_list>=5:
        
        orientation_diff_list=orientation_diff_list-3
        
    if save_correct==0 and orientation_diff_list<85:
        
        orientation_diff_list=orientation_diff_list+8
