...
def phase_0_lick_detection(serial_obj, timeout=30, valve_duration=100):
    # timeout = 30  # sec
    # send_data_until_confirmation(serial_obj=serial_obj, header_byte=WAIT_FOR_LICK_REWARDED,
    #                              data=[timeout * 10000, valve_duration * 1000])
    send_data_until_confirmation(serial_obj=serial_obj, header_byte=WAIT_FOR_LICK_REWARDED,
                                 data=[timeout * 1000])
...
def lick_detection(win, serial_obj, rewarded=True, timeout=120, valve_duration=100):
    stop = False
